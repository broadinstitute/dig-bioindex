...
orders by the column's default (case-insensitive) collation, the two orderings
disagree and page 2+ re-return keys already seen (duplicate keys). Ordering by
BINARY makes the SQL order match the Python cursor.
